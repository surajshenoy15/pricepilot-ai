...
public class RecommendationController {
    ...
    // ✅ NEW SAFE ROUTE - does not conflict with /{id}
    @PostMapping("/actions/generate-all")
    @Operation(summary = "Generate recommendations for all marketplace products of a tenant")
    public List<RecommendationResponse> generateAllRecommendations(
            @RequestParam Long tenantId
    ) {
        return recommendationService.generateAllRecommendations(tenantId);
    }
    ...
    public RecommendationResponse applyRecommendation(
            @PathVariable Long id
    ) {
